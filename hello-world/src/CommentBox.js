...
class CommentForm extends Component {
    render() {
        return (
            <form onSubmit={this._updateList.bind(this)}>
                <input name="Nombre" ref={(input) => {
                    this.name = input
                }}></input>
                <br/>
                <textarea name="Comment" ref={(lupe) => {
                    this.body = lupe
                }}></textarea>
                <input type="submit"/>
            </form>
    ...
    _updateList(event) {
        ...
        let name = this.name
        let body = this.body
        this.props.addComment(body.value, name.value)
    }
}
...
class CommentBox extends Component {
    ...
    _getComments() {
        return this.state.lista.reverse().map((comment) => {
            return (<Comment body={comment.body} name={comment.name} edad={comment.edad} key={comment.id}/>);
        });
    }
}
